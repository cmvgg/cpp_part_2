...
void PmergeMe::parser(const char *str, int const &len) const {
    if (len == 0 || str == NULL || str[0] == '\0')  // Verificación para cadenas vacías o NULL
        throw std::logic_error("Error: Empty or NULL input detected.");
    ...
    while (++i < len)
        if (!std::isdigit(str[i]))
            throw std::logic_error("Error: Invalid character in input.");
}
...
PmergeMe::PmergeMe(const char **av) {
    for (int i = 0; av[i]; i++) {
        ...
        if (tmp > INT_MAX)
            throw std::out_of_range("Error: Input number exceeds INT_MAX.");
        a.push_back(tmp);
        b.push_back(tmp);
    }

    if (a.size() < 2)
        throw std::runtime_error("Error: Not enough elements to process.");
